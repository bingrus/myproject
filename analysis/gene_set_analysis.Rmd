...
regulator.up <- regulator.selected[Direction == 'Up']

regulator.down <- regulator.selected[Direction == 'Down']

regulator.class[Regulator %in% regulator.up$Gene]

regulator.class[Regulator %in% regulator.down$Gene]

gene.age[GeneID %in% regulator.up$Gene]

gene.age[GeneID %in% regulator.down$Gene]

regulator.mutation[Hugo_Symbol %in% regulator.up$Gene]

regulator.mutation[Hugo_Symbol %in% regulator.down$Gene]
```

```{r plot,fig.width=12, fig.height=12,echo=FALSE,eval = FALSE}
...
regulator_fry_tb.skin[Pvalue <= 0.05 & Gene %in% selected.regulator]
```
...
reg.level <- levels(reg.col) 
reg.level
colfun <- colorRampPalette((c('white','yellow')))
levels(reg.col) <- colfun(11)
reg.col <- as.character(reg.col[selected.regulator])
names(reg.col) <- selected.regulator
selected.regulator.order <- selected.regulator[order(reg.col)]
```
...
```{r camera result}
genes.toanalyze <- regulator_fry_tb[Pvalue <= 0.05 & Gene %in% selected.regulator]$Gene
...
```{r camera heatmap selected,fig.width = 12,fig.height = 12,eval = FALSE}
#pdf('goslim_sub_old_order.pdf',width = 12,height = 12)
heatmap.2(toheatmap[order.2,],main = 'Camera analysis for GO slim processes',
          Rowv = FALSE,
          dendrogram="col",
          breaks = c(-1,-0.95,0,0.95,1),
          col = my_palette,trace="none",
...
```{r camera lung, include = FALSE}
camera.targetset.lung<-function(x){
gene<-x
n<-length(lung.cl)
cl.design <- matrix(nrow=n,ncol=2)
cl.design[,1] <- as.numeric(as.logical(mutation_df.lung[gene,]))
...
cl.design[,2]<-cl.design.2
colnames(cl.design)<-c('mutated','non_mutated')
r1 <- camera(ccle.exp[lung.cl], ids, cl.design,contrast=makeContrasts(mutated - non_mutated, levels=cl.design))
#Pvalue<-r1$Pvalue
#Direction<-r1$Direction 
#Pvalue<-1-Pvalue
#Pvalue[which(Direction == 'Down')]<- -Pvalue[which(Direction == 'Down')]
...
```{r camera result lung}
genes.toanalyze <- regulator_fry_tb.lung[Pvalue <= 0.05 & Gene %in% selected.regulator]$Gene
result.camera<-lapply(genes.toanalyze,camera.targetset.lung)
names(result.camera)<-genes.toanalyze
#save(result.camera,file = '/Users/ll/Desktop/work/analysis/analysis_on_expression/goslim_camera.Rdata')
```
...
```{r camera heatmap 2 lung,fig.width = 12,fig.height = 12}
#pdf('/Users/ll/Desktop/git/myproject/docs/figure/gene_set_analysis.Rmd/firstheatmap.pdf',width = 12,height = 12)
heatmap.2(toheatmap,main = 'Camera analysis for goslim processes for lung cancer cell lines',
          Rowv = FALSE,
          dendrogram="col",
          breaks = c(-1,-0.95,0,0.95,1),
          col = my_palette,trace="none",
...
```{r camera skin, include = FALSE}
camera.targetset.skin<-function(x){
gene<-x
n<-length(skin.cl)
cl.design <- matrix(nrow=n,ncol=2)
cl.design[,1] <- as.numeric(as.logical(mutation_df.skin[gene,]))
...
cl.design[,2]<-cl.design.2
colnames(cl.design)<-c('mutated','non_mutated')
r1 <- camera(ccle.exp[skin.cl], ids, cl.design,contrast=makeContrasts(mutated - non_mutated, levels=cl.design))
#Pvalue<-r1$Pvalue
#Direction<-r1$Direction 
#Pvalue<-1-Pvalue
#Pvalue[which(Direction == 'Down')]<- -Pvalue[which(Direction == 'Down')]
...
```{r camera result skin}
genes.toanalyze <- regulator_fry_tb.skin[Pvalue <= 0.05 & Gene %in% selected.regulator]$Gene
result.camera<-lapply(genes.toanalyze,camera.targetset.skin)
names(result.camera)<-genes.toanalyze
#save(result.camera,file = '/Users/ll/Desktop/work/analysis/analysis_on_expression/goslim_camera.Rdata')
```
