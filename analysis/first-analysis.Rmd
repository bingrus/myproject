...
cl.design[,2]<-cl.design.2
colnames(cl.design)<-c('mutated','non_mutated')
r1 <- fry(exprs(mydata)[,-which(toremove == TRUE)], idx, cl.design,contrast=makeContrasts(mutated - non_mutated, levels=cl.design),adjust.method = 'BH')
r1$Direction <- as.character(r1$Direction)
return(r1)}
#result.fry<-sapply(tofry,fry.targetset)
#result.fry<-t(as.data.frame(result.fry))
#result.fry.nona<-na.omit(result.fry)
#result.fry.nona<-as.data.frame(result.fry.nona)
load('/Users/ll/Desktop/work/analysis/analysis_on_expression/fryresult.Rdata')
result.fry.nona.p<-result.fry.nona[which(result.fry.nona$PValue<0.05),]
nrow(result.fry.nona.p)
result.fry.nona.p.20<-result.fry.nona.p[which(result.fry.nona.p$NGenes>20),]
result.fry.nona.pmix<-result.fry.nona[which(result.fry.nona$PValue.Mixed<0.05),]
result.fry.nona.pmix.20<-result.fry.nona.pmix[which(result.fry.nona.pmix$NGenes>20),]
#save(result.fry.nona,file = 'fryresult.Rdata')
result.fry.nona.p.20
result.fry.nona.pmix.20
```

* at least 5 mutated cell lines and 20 regulators

###Mix
```{r fryplot0,fig.width=10, fig.height=10,echo = FALSE}
par(mfrow=c(2,2))
for (gene in row.names(result.fry.nona.pmix.20)){
  plotbarcode(gene)
}
```

###Directional
```{r fryplot, fig.width=10, fig.height=10,echo = FALSE}
par(mfrow=c(5,2))
for (gene in row.names(result.fry.nona.p.20)){
  plotbarcode(gene)
}
```
##Camera Analysis
...
##ATG5 and MAML2 ssgsea 
  ...
  * plot 3 types (not-mutated-CNV,notmutated-notCNV,mutated)
...
```
##Result Discussion
* The resolution of the test p-value (fry(faster and more precise) mroast): 1) the roast has less resolution of the p-value as it is depended on the number of rotations 2) p-value changes from run to run
* Convert to z-score and run ssgsea 
* For mixed gene sets, test for absolute value
* Find IC50 for the inhibitor of the selected gene-cell line pair

##Meeting Summary
* Need to look more closely at the target sets
    - roast --- did fry
    - abs(logFC) 
        - ssgsea
    - ssgsea 
        - wilcox test between mcl-notmcl
* global analysis
    - DGE for all mcl-notmcl (which gene or gene set)
    - ssgsea for more precise pathway or process

* IC50
    - comparison between mcl-notmcl

* For thesis
    - what are the null hypothesis of roast or camera
