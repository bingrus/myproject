...
##Results of significant differential expressed regulators (logFC >= 1 or <= -1, adjusted p-value <= 0.05)
```{r data}
...
##Results of significant differential expressed regulators (logFC >= 1 or <= -1, adjusted p-value <= 0.05) of only selected regulators
```{r regulators }
## load selected regulators
load('./data/regulators_5_cl_20_tar.Rdata')
regulator_dge_tb[ID %in% selected.regulator & adj.P.Val <= 0.05 & (logFC >= 1 | logFC <= -1)]
```
...
regulator_dge_tb.lung[(logFC >= 1 | logFC <= -1) & adj.P.Val <= 0.05]
```
...
regulator_dge_tb.skin[(logFC >= 1 | logFC <= -1) & adj.P.Val <= 0.05]
```
