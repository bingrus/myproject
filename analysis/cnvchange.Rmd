...
compute cnv change by 1) extracting cell lines with mutated regulators, and the cell lines with non-mutated regulator. 2) count the cnv change by setting cutoff based on the data in all cell lines and all genes 3) compute the ratio of cnv change that is higher in not mutated cells lines of a regulator
...
plot(density(CNV_cell_lines_m$CNV))
...
plot(a,main = 'Distribution of the CNV change ratio per regulator')
```
...
plot(density(log(cnvchange.5.tar)))
...
## age of regulators
```{r regulator age}
hist(gene.age[GeneID %in% regulator.selected]$Phylostrata, main = 'Regulator Phylostrata', xlab = 'phylostrata')
```

## class of regulators
```{r regulator class}
barplot(table(regulator.class[Regulator %in% regulator.selected]$Regulator_class), main = 'Regulator class', xlab = 'class',ylab = 'number')
```
```{r save result, eval = FALSE}
save(regulator.selected,file = '../data/cnv_result.Rdata')
